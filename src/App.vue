<template>
      ...
      <div class="avatar-wave-jieting" v-if="showJieting && othersInfo">
        <div class="ripple-wave">
          <div class="ripple-wave-2">
            <img
              class="avatar"
              :src="'assets/avatar/avatar' + othersInfo.avatar"
            />
          </div>
        </div>
        <div class="person-one">{{ othersInfo.name }}</div>
        <h1 class="person-one" v-show="showJieting">來電</h1> 
      </div>
      <video
        v-show="localStream"
        class="localvideo"
  ...
  <div class="room">
    <div class="avatar-wave-top">
      <div class="ripple-wave">
        <div class="ripple-wave-2">
          <img class="avatar" :src="'assets/avatar/avatar' + info.avatar" />
        </div>
      </div>
    ...
    <div v-if="full" style="color: #fff; font-size: 22px">房間人數已滿..</div>
    <div
      v-if="!full"
      :class="['call', 'bubbly-button', othersInfo ? '' : 'disabled']"
      @click="makeCall"
    >
      {{ othersInfo ? "Connect" : "等待另一位使用者進入房間..." }}
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      state: "init",
      room: "person2",
      showVideo: false, // 顯示鏡頭畫面
      showJieting: false, // 顯示接聽畫面
      showGuaduan: false, // 顯示掛斷畫面
      remoteStream: null, // 本機視訊串流
      localStream: null, // 遠端視訊串流
      full: false, // 房間滿人
      pcConfig: {
        // webrtc建立點對點的對等連接，需要用到iceServers，否則只能在內部網路使用
        iceServers: [{ url: "stun:192.168.1.117" }],
      },
    };
  },
  watch: {
  ...
  mounted() {
    ...
    if (process.env.NODE_ENV == "development") {
      url = "http://192.168.1.117:3003/";
    } else {
      url = "https://192.168.1.117:3003/";
    }
